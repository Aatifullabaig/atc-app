...
export type Runway = '04' | '22';
export type PatternLeg = 'upwind' | 'crosswind' | 'downwind' | 'base' | 'final' | 'deadside';

export interface PatternSpec {
  radial_deg: number;
  heading_deg: number;
  default_distance_nm: number;
  default_altitude_ft: number;
  inbound_outbound: 'inbound' | 'outbound';
  phase: 'airborne' | 'approach';
  label: string;
}

// RWY 22 Right-Hand Circuit Pattern
export const PATTERN_22_RIGHT: Record<PatternLeg, PatternSpec> = {
  upwind: {
    radial_deg: 225,
    heading_deg: 225,
...
export const PATTERN_04_RIGHT: Record<PatternLeg, PatternSpec> = {
  upwind: {
    radial_deg: 45,
    heading_deg: 45,
    default_distance_nm: 3,
 ...
 */
export function getPatternFor(runway: Runway, leg: PatternLeg): PatternSpec {
