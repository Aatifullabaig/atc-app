...
import { PatternLeg, Runway, getPatternFor } from './patterns';

/**
 * Record a pattern leg for CL (circuit training) flights
 */
export async function recordPatternLeg(flightId: string, leg: PatternLeg): Promise<boolean> {
  try {
    // Get current flight
    ...
    const runway = (flight.runway_in_use || '22') as Runway;
 ...
 * Record go-around
 */
export async function recordGoAround(flightId: string): Promise<boolean> {
  try {
    const { data: flight, error: fetchError } = await supabase
      .from('flights')
 ...
 * Record landing
 */
export async function recordLanding(flightId: string): Promise<boolean> {
  try {
    const { data: flight, error: fetchError } = await supabase
      .from('flights')
 ...
 */
export async function startFlight(flightId: string): Promise<boolean> {
  try {
    const { error: updateError } = await supabase
      .from('flights')
      .update({
 ...
 */
export async function taxiToPoint(flightId: string, point: string): Promise<boolean> {
  try {
 ...
 * Record takeoff
 */
export async function recordTakeoff(flightId: string): Promise<boolean> {
  try {
    const { error: updateError } = await supabase
      .from('flights')
      .update({
 ...
 */
export async function recordShutdown(flightId: string): Promise<boolean> {
  try {
    const { error: updateError } = await supabase
      .from('flights')
      .update({
