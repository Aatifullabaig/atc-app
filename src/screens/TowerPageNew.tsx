...
import { recordPatternLeg, recordPositionReport, recordGoAround, recordLanding, startFlight, taxiToPoint, recordTakeoff, recordShutdown } from '../lib/flightActions';
import { Runway, PatternLeg, PATTERN_22_RIGHT, PATTERN_04_RIGHT } from '../lib/patterns';
import '../shared/TowerPage.css';

const TowerPage = () => {
  const [runway, setRunway] = useState<Runway>('22');
  const [groundOpsFlights, setGroundOpsFlights] = useState<any[]>([]);
  const [airOpsFlights, setAirOpsFlights] = useState<any[]>([]);
  ...
  const [postLandingFlightId, setPostLandingFlightId] = useState<string | null>(null);
  const [selectedTaxiPoint, setSelectedTaxiPoint] = useState<string | null>(null);
  const [manualPositionForm, setManualPositionForm] = useState({ radial: '', distance: '', altitude: '', direction: 'inbound' as 'inbound' | 'outbound' });
  ...
  const handlePatternLeg = async (flightId: string, leg: PatternLeg) => {
    try {
  ...
  const handleManualPosition = async (flightId: string) => {
    try {
  ...
  const handleGoAround = async (flightId: string) => {
    try {
  ...
  const handleLanding = async (flightId: string) => {
    try {
  ...
  const handleStart = async (flightId: string) => {
    try {
  ...
  const handleTaxiToPoint = async (flightId: string, point: string) => {
    try {
  ...
  const handleTakeoff = async (flightId: string) => {
    try {
  ...
  const handleShutdown = async (flightId: string) => {
    try {
  ...
  const isCircuitTraining = (flight: any) => flight.type_of_flight === 'CL';
  const isAfterLanding = (flight: any) => flight.phase === 'on_ground' && flight.last_landed_at && flight.landing_count > 0;
  const isTaxiReady = (flight: any) => flight.phase === 'on_ground' && !flight.last_landed_at;
                    ...
                    {isCircuitTraining(flight) ? (
                      <>
                        <div className="pattern-buttons">
                          {Object.keys(runway === '04' ? PATTERN_04_RIGHT : PATTERN_22_RIGHT).map((leg: any) => (
                            <button
                              key={leg}
        ...
        <div className="modal-overlay" onClick={() => setSelectedFlight(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <select value={manualPositionForm.direction} onChange={e => setManualPositionForm({ ...manualPositionForm, direction: e.target.value as any })}>
